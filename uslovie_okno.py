...
def usl_okno(player, icon_png, icon_ico, players, code, ip, text):
    ...
    def make_net(attr):
        if attr == 'Поменяться': comanda = lambda p1=f'igrok{player}', p2=f'igrok{mesto[index][play_index]}', char=text.split()[1]: deal_char(p1,p2,char)
        elif attr == 'Возраст': comanda = lambda p2=mesto[index][play_index]:(requests.post(f'http://{ip}/rooms/{code}/uslovie/age', json={'player1':player, 'player2':p2}), okno.destroy())
        elif attr == 'Беременность': comanda = lambda p2=mesto[index][play_index]:(requests.post(f'http://{ip}/rooms/{code}/uslovie/child', json={'player1':player, 'player2':p2}), okno.destroy())
