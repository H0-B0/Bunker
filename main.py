...
def get_app_data_path():
    """Возвращает путь к папке AppData/Local/BunkerGame"""
    if hasattr(sys, '_MEIPASS'):
        # Для exe: используем AppData/Local/BunkerGame
        app_data_path = os.path.join(os.environ['LOCALAPPDATA'], 'BunkerGame')
    else:
        # Для разработки: используем папку рядом с скриптом
...
def setup_resources():
    """Настраивает ресурсы и возвращает пути"""
    app_data_path = get_app_data_path()
    
    # ⭐ АВТОМАТИЧЕСКИ находим .db файл в ресурсах
    ...
    # Берем первый .db файл (base.db, base2.db, и т.д.)
    DB_NAME = db_files[0]
    
    # Пути к файлам в AppData
    icon_ico_path = os.path.join(app_data_path, 'bunker3.ico')
    icon_png_path = os.path.join(app_data_path, 'bunker3.png')
    db_path = os.path.join(app_data_path, DB_NAME)
    
    # Всегда копируем БД заново (перезаписываем)
    source_db = get_resource_path(DB_NAME)
    shutil.copy2(source_db, db_path)
    print(f"БД скопирована: {DB_NAME}")
    print(f"Путь: {db_path}")
    
        # Копируем PNG иконку (для Linux)
    ...
    # Копируем ICO иконку (для Windows)
...
if os.path.exists(ICON_ICO_PATH if sys.platform.startswith('win') else ICON_PNG_PATH):
    if sys.platform.startswith('win'):
