...
def okno10(player, icon_png, icon_ico, db_path, code='', server_ip='127.0.0.1:8000'):
    
    print(f"okno10: db_path = {db_path}")
    print(f"okno10: db exists = {os.path.exists(db_path) if db_path else False}")
    print(f"okno10: server_ip = {server_ip}")
    ...
    try:
        ...
        def zamena():
            user1.count.config(text=f'{len(array)}/5 людей')
            user2.count.config(text=f'{len(array)}/5 людей') 
            user3.count.config(text=f'{len(array)}/5 людей')
            user4.count.config(text=f'{len(array)}/5 людей')
            user5.count.config(text=f'{len(array)}/5 людей')
            user6.count.config(text=f'{len(array)}/5 людей')
            user7.count.config(text=f'{len(array)}/5 людей')
            user8.count.config(text=f'{len(array)}/5 людей')
            user9.count.config(text=f'{len(array)}/5 людей')
            user10.count.config(text=f'{len(array)}/5 людей')
        ...
        user8 = Player(player8,8)
        user9 = Player(player9,9)
        user10 = Player(player10,10)
        ...
        def show_page(page):
            ...
            page8.pack_forget()  # прячем страницу 8
            page9.pack_forget()  # прячем страницу 9
            page10.pack_forget()  # прячем страницу 10
        ...
        btn8 = tk.Button(btn_frame, text="Игрок 8", command=lambda: show_page(page8), **NAV_BUTTON_STYLE)
        btn8.pack(side="left", padx=5)

        btn9 = tk.Button(btn_frame, text="Игрок 9", command=lambda: show_page(page9), **NAV_BUTTON_STYLE)
        btn9.pack(side="left", padx=5)

        btn10 = tk.Button(btn_frame, text="Игрок 10", command=lambda: show_page(page10), **NAV_BUTTON_STYLE)
        btn10.pack(side="left", padx=5)

        # 4. ПОКАЗЫВАЕМ ПЕРВУЮ СТРАНИЦУ ПРИ ЗАПУСКЕ
        assoc1 = {1:page1, 2:page2, 3:page3, 4:page4, 5:page5, 6:page6, 7:page7, 8:page8, 9:page9, 10:page10}
        assoc2 = {1:user1, 2:user2, 3:user3, 4:user4, 5:user5, 6:user6, 7:user7, 8:user8, 9:user9, 10:user10}
