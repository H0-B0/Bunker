...
#Основная функция
def okno9(player, icon_png, icon_ico, db_path, code='', server_ip='127.0.0.1:8000'):
    
    print(f"okno9: db_path = {db_path}")
    print(f"okno9: db exists = {os.path.exists(db_path) if db_path else False}")
    print(f"okno9: server_ip = {server_ip}")
    ...
    try:
        ...
        def zamena():
            user1.count.config(text=f'{len(array)}/4 людей')
            user2.count.config(text=f'{len(array)}/4 людей') 
            user3.count.config(text=f'{len(array)}/4 людей')
            user4.count.config(text=f'{len(array)}/4 людей')
            user5.count.config(text=f'{len(array)}/4 людей')
            user6.count.config(text=f'{len(array)}/4 людей')
            user7.count.config(text=f'{len(array)}/4 людей')
            user8.count.config(text=f'{len(array)}/4 людей')
            user9.count.config(text=f'{len(array)}/4 людей')
        ...
        class Player:
            ...
            def create(self):
                ...
                self.count = tk.Label(container,text=f'{len(array)}/4 людей',**HEADING_STYLE)
        ...
        assoc1 = {1:page1, 2:page2, 3:page3, 4:page4, 5:page5, 6:page6, 7:page7, 8:page8, 9:page9}
        assoc2 = {1:user1, 2:user2, 3:user3, 4:user4, 5:user5, 6:user6, 7:user7, 8:user8, 9:user9}
